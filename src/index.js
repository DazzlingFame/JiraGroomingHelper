...
const notifyAboutNotStudy = async () => {
  ...
  const messages = tasks.issues.map((t) => {
    const main_component = t.fields.components[0]
      ? t.fields.components[0].name
      : "Product";
...
const notifyAboutStudy = async () => {
  ...
  const messages = tasks.issues.map((t) => {
    const assignee = t.fields.assignee.emailAddress
      ? t.fields.assignee.emailAddress.split("@")[0]
      : "karepovals";
